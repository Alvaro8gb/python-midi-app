...
class Interface(tk.Tk):
    def __init__(self, controller):
        ...
        connect_button = tk.Button(self, text="Conectar MIDI", command=self.window_select_midi)
        connect_button.place(x=540, y=0)

        # Selector modelo
        self.model_selection = self.controller.get_model_name()
        mode_button = tk.Button(self, text="Modelo", command=self.window_select_model)
        mode_button.place(x=540, y=100)

        # Selector tamaño chunk
        tk.Label(self, text="chunk:", bg="white").place(x=520, y=40)
        self.spinbox = tk.Spinbox(self, values=[str(size) for size in CHUNK_SIZE], width=15,
